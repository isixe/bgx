...
export function ImageUploader({ disabled = false, disabledMessage }: ImageUploaderProps) {
  const { setOriginalImage, setError, isDarkMode, setBatchMode, addToBatchQueue, clearBatchQueue } =
    useAppStore();
  ...
  return (
    ...
    >
      ...
      <h3
        className={`mb-2 text-base font-medium ${isDarkMode ? 'text-slate-100' : 'text-slate-900'}`}
      >
