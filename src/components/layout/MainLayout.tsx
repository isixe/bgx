...
export function MainLayout() {
  ...
  return (
    <div className={`h-[100dvh] overflow-y-auto ${isDarkMode ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      {isModelDownloading && (
        <div
          className={`fixed top-16 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-md rounded-xl border p-4 shadow-lg ${isDarkMode ? 'border-indigo-500/30 bg-indigo-900/90' : 'border-indigo-200 bg-indigo-50'}`}
        >
          <div className="flex items-center gap-3">
            <svg
              className={`h-5 w-5 animate-spin ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-sm font-medium ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}
              >
          ...
          <div
            className={`mt-3 h-1.5 rounded-full overflow-hidden ${isDarkMode ? 'bg-slate-700' : 'bg-slate-200'}`}
          >
            <div
      ...
      <div className="[@media(min-width:900px)]:hidden">
        <main {...dragProps} className={`${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'} relative`}>
          {currentPage === 'settings' ? (
            <SettingsPage />
            ...
            >
              ...
              >
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span
                    className={`text-lg font-medium ${isDarkMode ? 'text-indigo-400' : 'text-indigo-600'}`}
                  >
                    {t('uploaderDrop')}
                  </span>
                </div>
      ...
      <div className={`space-y-4 ${isBatchProcessing ? 'pointer-events-none opacity-70' : ''}`}>
        ...
        {batchMode && batchQueue.length > 0 && (
          <div className="space-y-4">
            ...
            >
              {t('reprocess')}
              {selectedBatchItemIds.length > 0 ? ` (${selectedBatchItemIds.length})` : ''}
            </button>
